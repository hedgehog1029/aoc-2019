...
defmodule AOC do
	...
	def verify_double([digit | tail], last, beforethat) do
		if digit == last and digit != List.first(tail) and digit != beforethat do
			true
		else
			verify_double(tail, digit, last)
		end
	end

	def verify_double([], _last, _beforethat) do
		false
	end
	...
	def apply_to(num) do
		...
		verify_double(digits, 0, 0) and verify_increasing(digits, 0)
	end
